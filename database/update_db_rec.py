    ...
    while tbl_flag:
        ...
        if tbl_flag:
            ...
            while fltr_flag != '!*':
                ...
                contn = input("\nTo continue update process enter 'y':")
                if contn.lower() != 'y':
                ...
                while cln_flag:
                    ...
                    if cln_flag:
                        ...
                        if not cln_name:
                            ...
                            continue    # while cln_flag:  
                        #else:
                        print('Selected column to update:', cln_name)
                        print_tbl_clmn(fltr_rec, cln_name)

                        # confirm continue update process
                        contn = input("\nTo continue update process enter 'y':")
                        if contn.lower() != 'y':
                            print('Returning to enter column to update ...')
                            continue    #while cln_flag:
                        
                        # input new value
                        new_col_val = input('Enter new column value:')
                        # take old value to define type
                        old_col_val = getattr(fltr_rec[0], cln_name)
                        # if old value not string then eval
                        if not isinstance(old_col_val, str):
                            new_col_val = eval(new_col_val)

                        print('New value to set:', new_col_val)
                        # confirm continue update process
                        contn = input("\nTo continue update process enter 'y':")
                        if contn.lower() != 'y':
                            print('Returning to enter column to update ...')
                        ...
                        # set new value for all filtered records
                        set_tbl_clmn(fltr_rec, cln_name, new_col_val)
                        # print table with column content
                        print_tbl_clmn(fltr_rec, cln_name)

                        # confirm continue update process
                        contn = input("\nTo commit updates enter 'y':")
                        if contn.lower() != 'y':
                            print('Returning to enter column to update ...')
                            continue    #while cln_flag:
